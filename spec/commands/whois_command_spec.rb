...
describe WhoisCommand do
  ...
  it "should find users from my channels" do
    ...
    irc_connection.should_receive(:channels).and_return({'irc/ottotest' => channel})
    irc_connection.should_receive(:find_user_by_nick).with('ottomob').and_return(user)
    irc_connection.should_receive(:send_reply).with(/Ottomob Heelskae.*#irc\/ottotest.*:seconds idle.*End of WHOIS/m)
  ...
  it "should be invalid without arguments" do
    irc_connection = mock(:irc_connection, :registered? => true, :authenticated? => true)
    cmd = WhoisCommand.new(irc_connection)
    cmd.set_data([])
